            ...
            for i in tqdm(range(0, num_frames, batch_size), desc="Processing batches"):
                ...
                for key in ['pose', 'betas', 'trans', 'vertices3d']:
                    if key in pred:
                        ...
                        # NLF returns tensors with shape (batch_size, 1, N)
                        # We need to squeeze to get (batch_size, N)
    ...
    def get_first_frame_data(self, frames: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for key in ['pose', 'betas', 'trans', 'vertices3d']:
            if key in pred:
                ...
                # NLF returns tensors with shape (1, N) for single frames
                # We need to squeeze to get (N,)
